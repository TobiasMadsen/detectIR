...
List detectImperfectIR_cpp(IntegerVector enc1, IntegerVector enc2, int minLen, int maxLen, int maxMismcNum, int maxLoopLen = 0) {
  ...
  for(int i = 0 ; i < N-minLen+1; ++i){
    ...
    if( dist(cumScore[minLen+i], cumScore[i]) < maxMismcNum ){
      // TODO: Calculate actual mismatch number (possibly not counting a central loop)
      int misMatchNum = 0;
      int firstMatch = -1;
      int lastMatch = -1;
      for(int j = minLen/2-1+i; j >= i; --j){
	bool match = isZero(nseq[j] + nseq[2*i-j+minLen-1]);
	misMatchNum += match?0:2;
	if(match){
	  firstMatch = j;
	  lastMatch  = 2*i-j+minLen-1;
	}
      }
      if(misMatchNum <= maxMismcNum)
	candidates.push_back(Candidate(i, i+minLen-1, misMatchNum, firstMatch, lastMatch));
    }
  }
  
  // Add candidates with distance minLen+1
  // TODO: Refactor?
  for(int i = 0 ; i < N-minLen; ++i){
    // Loop over cummulative score
    if( dist(cumScore[minLen+1+i], cumScore[i]) < maxMismcNum ){
      // TODO: Calculate actual mismatch number (possibly not counting a central loop)
      int misMatchNum = 0;
      int firstMatch = -1;
      int lastMatch = -1;
      for(int j = minLen/2-1+i; j >= i; --j){
	bool match = isZero(nseq[j] + nseq[2*i-j+minLen+1-1]);
	misMatchNum += match?0:2;
	if(match){
	  firstMatch = j;
	  lastMatch  = 2*i-j+minLen+1-1;
	}
      }
      if(misMatchNum <= maxMismcNum)
	candidates.push_back(Candidate(i, i+minLen-1, misMatchNum, firstMatch, lastMatch));
